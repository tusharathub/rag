"use client";

import * as React from "react";
import Link from "next/link";
import { usePathname } from "next/navigation";
import {
  LayoutDashboard,
  MessageSquare,
  FolderLock,
  Layers,
  ChevronLeft,
  ChevronRight,
  Upload,
  Cpu,
} from "lucide-react";
import { useAppSelector, useAppDispatch } from "@/store";
import { toggleSidebar, setUploadModalOpen } from "@/store/slices/uiSlice";
import { cn } from "@/utils/cn";
import { SignedIn, SignedOut, UserButton, SignInButton } from "@clerk/nextjs";

export function MainSidebar() {
  const dispatch = useAppDispatch();
  const pathname = usePathname();
  const sidebarCollapsed = useAppSelector((state) => state.ui.sidebarCollapsed);

  const handleToggleSidebar = () => dispatch(toggleSidebar());
  const handleSetUploadModalOpen = (open: boolean) => dispatch(setUploadModalOpen(open));

  const navItems = [
    { id: "dashboard", label: "Dashboard", href: "/workspace/dashboard", icon: LayoutDashboard },
    { id: "chat", label: "AI Chat", href: "/workspace/chat", icon: MessageSquare },
    { id: "library", label: "Doc Library", href: "/workspace/library", icon: FolderLock },
    { id: "collections", label: "Collections", href: "/workspace/collections", icon: Layers },
  ] as const;

  return (
    <aside
      className={cn(
        "h-screen bg-[#080808] border-r border-slate-900 text-slate-200 transition-all duration-300 flex flex-col justify-between z-30 font-mono",
        sidebarCollapsed ? "w-16" : "w-64"
      )}
    >
      <div>
        {/* Top Header Logo */}
        <div className="h-16 flex items-center justify-between px-4 border-b border-slate-900">
          <Link href="/" className="flex items-center gap-2 overflow-hidden group">
            <div className="h-8 w-8 rounded bg-[#FFA028] flex items-center justify-center flex-shrink-0 text-slate-950 font-bold">
              <Cpu className="h-4 w-4" />
            </div>
            {!sidebarCollapsed && (
              <span className="font-bold text-base tracking-tight text-white truncate">
                RAG<span className="text-[#FFA028]">//</span>ENGINE
              </span>
            )}
          </Link>

          {!sidebarCollapsed && (
            <button
              onClick={handleToggleSidebar}
              className="text-slate-500 hover:text-white hover:bg-slate-900 p-1 rounded transition-colors hidden md:block"
            >
              <ChevronLeft className="h-4 w-4" />
            </button>
          )}
        </div>

        {/* Action Button: Upload */}
        <div className="p-3">
          <button
            onClick={() => handleSetUploadModalOpen(true)}
            className={cn(
              "w-full flex items-center justify-center gap-2 bg-[#FFA028] hover:bg-[#E58D1B] text-slate-950 font-bold transition-all clip-chamfer shadow-md",
              sidebarCollapsed ? "p-2" : "px-4 py-2.5"
            )}
          >
            <Upload className="h-4 w-4" />
            {!sidebarCollapsed && <span className="font-mono text-xs uppercase tracking-wider">UPLOAD FILE</span>}
          </button>
        </div>

        {/* Navigation list */}
        <nav className="px-2 py-3 space-y-1">
          {navItems.map((item) => {
            const isActive = pathname === item.href || (item.id === "dashboard" && pathname === "/workspace");
            const Icon = item.icon;
            return (
              <Link
                key={item.id}
                href={item.href}
                className={cn(
                  "w-full flex items-center gap-3 px-3 py-2.5 rounded text-xs font-mono transition-all group duration-150",
                  isActive
                    ? "bg-[#FFA028] text-slate-950 font-bold shadow-md shadow-[#FFA028]/20"
                    : "text-slate-400 hover:text-white hover:bg-slate-900"
                )}
              >
                <Icon
                  className={cn(
                    "h-4 w-4 flex-shrink-0 transition-transform duration-200",
                    isActive ? "text-slate-950" : "group-hover:scale-110 text-slate-400 group-hover:text-slate-200"
                  )}
                />
                {!sidebarCollapsed && <span className="truncate tracking-wider">{item.label}</span>}
              </Link>
            );
          })}
        </nav>
      </div>

      {/* Sidebar Footer */}
      <div className="p-3 border-t border-slate-900 flex flex-col gap-3">
        {/* Toggle Collapse on small width */}
        {sidebarCollapsed && (
          <button
            onClick={handleToggleSidebar}
            className="mx-auto text-slate-500 hover:text-white p-2 rounded hover:bg-slate-900 hidden md:block"
          >
            <ChevronRight className="h-4 w-4" />
          </button>
        )}

        {/* Profile Card / Controls */}
        <div className={cn("flex items-center justify-between gap-2", sidebarCollapsed && "justify-center")}>
          <SignedIn>
            <div className="flex items-center gap-2 overflow-hidden">
              <UserButton
                afterSignOutUrl="/sign-in"
                appearance={{
                  elements: {
                    avatarBox: "h-8 w-8",
                  },
                }}
              />
            </div>
          </SignedIn>
          <SignedOut>
            {!sidebarCollapsed && (
              <SignInButton mode="modal">
                <button className="text-xs text-[#FFA028] hover:underline font-mono">
                  Sign In
                </button>
              </SignInButton>
            )}
          </SignedOut>
          {!sidebarCollapsed && (
            <span className="text-[10px] text-slate-600 font-mono">V2.4</span>
          )}
        </div>
      </div>
    </aside>
  );
}
